      ...
      >
        {words.slice(0, Math.min(words.length, 5)).map((word, index) => {
          ...
          return (
            <div
              key={`fan-${index}`}
              className={`absolute rounded-lg border-2 shadow-xl transition-all duration-500 ease-out ${bgColor} ${borderColor}`}
              style={{
                width: 128,
                height: 160,
                transform: `
            ...
            >
              <div className="absolute inset-0 flex flex-col items-center justify-center p-3">
                <div
                  className={`text-2xl font-normal ${colors.ui.text.slate} mb-1`}
                >
                  {word.word}
                </div>
                <div className={`text-sm ${colors.ui.text.blue} mb-1`}>
                  {word.reading}
                </div>
                <div
                  className={`text-xs ${colors.ui.text.slate} text-center px-1 leading-tight`}
                >
                  {word.meaning}
                </div>
              </div>
